...
export default function RegisterForm() {
    ...
    async function registerAccount(data: any) {
        setIsLoading(true);
        await UserService.register(data.email, data.password, data.confirmPassword).then((result) => {
    ...
    return (
        <form className={"flex flex-col space-y-4 w-full px-6"} onSubmit={handleSubmit(registerAccount)}>
            <div className={"flex flex-col"}>
                ...
                <div className={"w-full h-fit bg-[#333333] rounded-md py-2 px-6 flex flex-col mb-2"}>
                    <label className={"text-[#717171] font-poppins text-xs"}>Confirm Password</label>
                    <input className={"text-white font-poppins bg-transparent pb-[0.25] h-6"} type={"password"}
                           spellCheck={false} required minLength={10} maxLength={72}

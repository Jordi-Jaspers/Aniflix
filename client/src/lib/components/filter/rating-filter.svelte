		...
		<div class="grid gap-4">
			<div class="space-y-2">
				...
				<p class="text-sm text-muted-foreground">
					Set the rating to find anime with a specific rating.
				</p>
			</div>
			<div class="grid gap-2">
				<div class="grid grid-cols-3 items-center gap-4">
					<Label for="minRating">Min</Label>
					<Input type="number" id="minRating" max="10" placeholder="7.0" step=0.1 bind:value={minRating} class="col-span-2 h-8" />
				</div>
				<div class="grid grid-cols-3 items-center gap-4">
					<Label for="maxRating">Max</Label>
					<Input type="number" id="maxRating" max="10" placeholder="10.0" step=0.1 bind:value={maxRating} class="col-span-2 h-8" />
				</div>
			</div>
		</div>
	</Popover.Content>

...
<div class="flex flex-col ">
    <div class="relative aspect-[420/600] w-auto h-full rounded-t-[0.75rem] overflow-hidden">
            ...
            <div class="w-full max-h-full space-y-1">
                ...
                <div class="flex justify-between space-x-2 items-center">
                    <h4 class="text-sm h-[2.5rem] overflow-hidden">
                        {result.title.toLowerCase().split(' ').map((s) => s.charAt(0).toUpperCase() + s.substring(1)).join(' ')}
                    </h4>
                </div>
            </div>
        </div>

...
<Dialog.Root bind:open={$useShowChangelog}>
	...
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title class="flex flex-col items-center space-y-2">
				<MessageCircleCode class="h-10 w-10" />
				<span class="font-semibold">What's new?</span>
				<span class="mt-1 text-[0.75rem] font-light italic text-muted-foreground"> Version: {applicationVersion} </span>
			</Dialog.Title>

			<Dialog.Description class="space-y-4 p-4">
				<div class="space-y-2">
					<h4 class="text-lg text-primary-foreground">New</h4>
				...
				<div class="space-y-2">
					<h4 class="text-lg text-primary-foreground">Fixed</h4>
					<ul class="ml-8 list-disc space-y-1">
		...
		<Dialog.Footer>
			<Button on:click={() => $useShowChangelog = false}>Close</Button>
		</Dialog.Footer>
	</Dialog.Content>
</Dialog.Root>

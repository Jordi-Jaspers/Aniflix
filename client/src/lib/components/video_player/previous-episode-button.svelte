...
{#if episode}
    <button class="relative h-full w-fit items-center space-x-2 hover:scale-125 flex {showPrevEpisodeButton ? 'opacity-100' : 'opacity-0'}"
            aria-label="Previous Episode" disabled={!showPrevEpisodeButton} on:click={navigate}>

...
import type {OAuthUser} from "../../interfaces/OAuthUser";
...
export default class UserService {
    ...
    static requestVerificationEmail(email: string): Promise<{ isSuccess: boolean, isError: boolean, error: string }> {
        ...
        return pocketBase.collection("users").requestVerification(email).then(() => {
            return {
                isSuccess: true,
                isError: false,
                error: "Email sent successfully, Check your email for verification"
            };
        }).catch((e: any) => {
    ...
    static async register(email: string, password: string, confirmPassword: string): Promise<{
        isError: boolean,
        error: string
    }> {

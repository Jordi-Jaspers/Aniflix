...
export default function EpisodeCards({className, episodes}: Props) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        list.push(
            <li key={i}
                className={"px-4 flex justify-start space-x-8 w-full h-auto items-start hover:bg-[#1a1920] rounded"}
                onClick={() => setPage(i)}>
                <span className={"block text-white font-poppins min-w-[75px] w-[30%]"}>Page {i}</span>
                <span
                    className={"block text-white font-poppins font-light text-sm min-w-fit w-[70%]"}>(Episodes {lowerBound(i)} - {upperBound(i)})</span>
            </li>
        );
    }

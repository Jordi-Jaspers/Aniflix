...
export default function VideoControls({className}: Props) {
    ...
    const {
        ...
        duration,
        currentTime
    } = useRecoilValue(videoPlayerState);
    ...
    return (
        <>
            ...
            <div
                className={`${className ? className : ''} fixed top-0 bottom-0 h-screen w-screen bg-black/20 pt-4 z-10`}
                    ...
                    <div className="flex justify-between items-end py-2 h-full">
                        <div className="flex items-end justify-evenly space-x-4">
                            <button className="videoPlayerControls" onClick={() => setIsPlaying(!isPlaying)}
                                    ref={buttonRef}>
                                {isPlaying ? <PauseIcon className={"h-full w-full"}/> :
                        ...
                        <div className="hidden sm:block text-white text-sm text-center font-poppins items-end">
                            <p className="font-bold text-lg">{(anime?.title.romaji ? anime.title.romaji : anime?.title.english)}</p>
                        ...
                        <div className="flex items-end justify-evenly space-x-2 sm:space-x-4">
                            <button className="videoPlayerControls" onClick={() => nextEpisode()}>
                                <Image width={64} height={64} src={NextEpisode} alt={"Next Episode"}
